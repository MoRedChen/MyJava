...
public class posFXMLController implements Initializable {
    ...
    public void initMyComponents() {
        ...
        size.getItems().setAll("SS", "S", "M", "L", "XL", "XXL");
        ...
        TableColumn<OrderDetail, String> order_item_size = new TableColumn("尺寸");
        order_item_size.setCellValueFactory(new PropertyValueFactory("size"));

        table.setEditable(true);

        table.getColumns().addAll(order_item_name, order_item_price, order_item_qty, order_item_size);
        ...
        menuPane.getChildren().add(getCategoryMenu("衣服"));
    ...
    public ScrollPane getCategoryMenu(String category) {
...
//        ScrollPane menuScroll = new ScrollPane();
        ...
        menuGrid.setVgap(10); 
        menuGrid.setStyle("-fx-background-color: #C7C7E2;");
...
//                productPane.getStyleClass().add("productShowBlock");
        ...
        scrollPane.setStyle("-fx-background-color: #C7C7E2; -fx-pref-height:657; -fx-pref-width:543;");
//        return menuPane;
        return scrollPane;
//        return menuGrid;
...
//        itemPane.setStyle(css); //套用樣式
        itemPane.getStyleClass().add("productShowBlock");
    ...
    private void add_to_cart(ActionEvent event) {
        ...
        String s = size.getSelectionModel().getSelectedItem();
        ...
        if (duplication) {
            System.out.println(prod_id + " (" + s + ") " + "已經加入購物車");
        } else {
    ...
    private void select_menu(ActionEvent event) {
        ...
        if (category.equals("衣服")) {
            menuPane.getChildren().addAll(getCategoryMenu("衣服"));
        } else if (category.equals("褲子")) {
            menuPane.getChildren().addAll(getCategoryMenu("褲子"));
        } else if (category.equals("外套")) {
            menuPane.getChildren().addAll(getCategoryMenu("外套"));
        }
    }

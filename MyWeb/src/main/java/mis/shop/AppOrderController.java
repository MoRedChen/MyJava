...
public class AppOrderController {
    ...
    public String home(Model model) {
        ...
        List<Product> products = productDao.findByCate("衣服"); //只有衣服類
    ...
    public String addToCart(@ModelAttribute(name = "prod_id") String prod_id, @ModelAttribute(name = "size") String size, @ModelAttribute(name = "quantity") String quantity, Model model) {
...
//        return "home.html";
    ...
    public String modifyCartSize(@RequestParam String prod_id, @RequestParam String prod_size, @RequestParam("orderSize") String orderSize) {
        ...
        for (int i = 0; i < cart.size(); i++) {
            if (cart.get(i).getProduct_id().equals(prod_id)) {
                if (cart.get(i).getSize().equals(prod_size)){
                    //檢查購物車List是否已經有這項產品
                    for (OrderDetail od : cart) {
                        if (cart.get(i).getProduct_id().equals(prod_id)) {
                            if (od.getSize().equals(orderSize)) {
                                break;
                            }
                            else {
                                //找到該項產品 數量更新為orderQty值 後面的迴圈就跳開不必做了
                                cart.get(i).setSize(orderSize);
                                break;
                            }
                        }
                    }
